...
const Strip3DView = dynamic(() => import("./Strip3DView"), {
  ssr: false,
  loading: () => (
...
export default function StripBuilder({ open, pointIds, onClose }: StripBuilderProps) {
  ...
  const [activeTab, setActiveTab] = useState<"edit" | "3d">("edit");
  ...
  }, [pointIds, overrides, setActiveTab]);
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="flex h-[88vh] w-[min(1100px,94vw)] flex-col overflow-hidden rounded-xl border border-neutral-700 bg-neutral-900 text-neutral-100 shadow-2xl">
        ...
        {/* Tab Bar (only when result is available) */}
        {result && (
          <div className="flex border-b border-neutral-800 bg-neutral-950/20 px-5">
            <button
              onClick={() => setActiveTab("edit")}
              className={`border-b-2 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider transition-colors cursor-pointer ${
                activeTab === "edit"
                  ? "border-cyan-500 text-white"
                  : "border-transparent text-neutral-400 hover:text-neutral-200"
              }`}
            >
              Edit Rectification Lines
            </button>
            <button
        ...
        <div className="flex min-h-0 flex-1">
          {activeTab === "3d" && result ? (
            <Strip3DView gcsPrefix={result.gcsPrefix} />
          ) : (
            <>
              {loading && (
